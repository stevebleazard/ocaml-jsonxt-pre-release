...
let command_dump idx sys_argv_len =
  ...
  let dump =
    try begin
      ...
      | `Yojson_basic -> let json = Jsonxt.Yojson.Basic.of_string contents in Jsonxt.Tools.json_to_string_repr json
      | `Yojson_safe -> let json = Jsonxt.Yojson.Safe.of_string contents in Jsonxt.Tools.json_to_string_repr json
    end
    with
