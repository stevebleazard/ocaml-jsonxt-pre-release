{
  ...
  let unescape_string s =
    ...
    for i = 0 to l - 1 do
      ...
      | `Escape -> begin
        match s.[i] with
         | '"'  -> Bytes.unsafe_set s' !j '"';    state := `Char
         | '\\' -> Bytes.unsafe_set s' !j '\\';   state := `Char
         | 'b'  -> Bytes.unsafe_set s' !j '\b';   state := `Char
         | 'f'  -> Bytes.unsafe_set s' !j '\012'; state := `Char
         | 'n'  -> Bytes.unsafe_set s' !j '\n';   state := `Char
         | 'r'  -> Bytes.unsafe_set s' !j '\r';   state := `Char
         | 't'  -> Bytes.unsafe_set s' !j '\t';   state := `Char
         | 'u'  -> state := `U1_h1
